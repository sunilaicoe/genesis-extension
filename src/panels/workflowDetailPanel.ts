...
export class WorkflowDetailPanel {
    ...
    private _handleRunPipeline() {
        if (this._isLocal) {
            ...
            if (localWf?.cloudWorkflowId) {
                // This local workflow was sourced from cloud — re-run the cloud pipeline
                vscode.window.showInformationMessage('Re-running cloud pipeline... Documents will be refreshed from AI output.');
                this._service.startPolling(localWf.cloudWorkflowId);
            } else {
                vscode.window.showInformationMessage('This workflow was generated from local templates. Create a new workflow to get AI-generated documents.');
            }
        } else {
    ...
    private _handleRegenerate(docType: string) {
        if (this._isLocal) {
            const localWf = this._service.getLocalWorkflow(this._workflowId);
            if (localWf?.cloudWorkflowId) {
                // Regenerate from cloud pipeline
                this._service.regenerateDocument(localWf.cloudWorkflowId, docType);
            } else {
                vscode.window.showInformationMessage('Create a new workflow to get AI-generated documents.');
            }
        } else {
    ...
    private _handleResetPipeline() {
        if (this._isLocal) {
            const localWf = this._service.getLocalWorkflow(this._workflowId);
            if (localWf?.cloudWorkflowId) {
                vscode.window.showInformationMessage('Re-running cloud pipeline...');
                const cloudId = localWf.cloudWorkflowId;
                this._service.getApi().resetPipeline(cloudId).then(() => this._service.startPolling(cloudId));
            } else {
                vscode.window.showInformationMessage('Create a new workflow to get AI-generated documents.');
            }
        } else {
        ...
        vscode.window.showInputBox({ prompt: 'Describe the changes you want to make:', placeHolder: 'e.g. Add authentication section, update the pricing model...' }).then(summary => {
            if (summary) {
                if (this._isLocal) {
                    const localWf = this._service.getLocalWorkflow(this._workflowId);
                    if (localWf?.cloudWorkflowId) {
                        this._service.continueWorkflow(localWf.cloudWorkflowId, summary);
                    } else {
                        vscode.window.showInformationMessage('Create a new workflow with updated requirements.');
                    }
                } else {
