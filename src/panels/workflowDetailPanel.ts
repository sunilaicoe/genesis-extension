    ...
    constructor(panel: vscode.WebviewPanel, service: WorkflowService, workflowId: string, name: string) {
        ...
        this._panel.webview.onDidReceiveMessage(async message => {
            switch (message.command) {
                ...
                case 'view-document': if (WorkflowDetailPanel._onViewDocument) WorkflowDetailPanel._onViewDocument(this._workflowName, workflowId, message.doc); break;
                ...
                case 'export': if (WorkflowDetailPanel._onExport) WorkflowDetailPanel._onExport(workflowId, this._workflowName); break;
                case 'new-project': if (WorkflowDetailPanel._onNewProject) WorkflowDetailPanel._onNewProject(); break;
                case 'run-pipeline': this._service.startPipeline(workflowId); break;
                case 'regenerate': this._service.regenerateDocument(workflowId, message.doc); break;
                case 'reset-pipeline': this._service.getApi().resetPipeline(workflowId).then(() => this._service.startPolling(workflowId)); break;
                case 'continue-workflow':
                    vscode.window.showInputBox({ prompt: 'Describe the changes you want to make:', placeHolder: 'e.g. Add authentication section, update the pricing model...' }).then(summary => {
                        if (summary) this._service.continueWorkflow(workflowId, summary);
                    });
                    break;
                case 'save-name':
                    if (message.name && message.name.trim()) {
                        const newName = message.name.trim();
                        this._workflowName = newName;
                        await this._service.getApi().updateWorkflowDetails(workflowId, newName, undefined);
            ...
            <div class="page">
                ...
                <div class="progress-section">
                    <div class="progress-left">
                        ...
                        <h2>${wfStatus === 'running' ? (agentDisplay ? `Active: ${agentDisplay}` : progressMsg) : progressMsg}</h2>
                    </div>
                ...
                <div class="action-bar">
                    ...
                    ${wfStatus === 'completed' ? `<button class="act-btn ab-primary" onclick="handleAction('export')"><span class="material-symbols-outlined">ios_share</span> Export Documents</button>` : ''}
